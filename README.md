...
> **注意**：受限于浏览器安全策略，通过拖拽安装的非官方 `.crx` 扩展有可能会被浏览器自动禁用。如果遇到这种情况，请改用**方法一**。
...
启用并保存后，扩展会立即检查一次登录状态，之后按随机间隔在后台运行。

“前台认证页和选课站自动登录”开关独立控制用户主动打开统一身份认证页或选课网站时的自动登录行为。
