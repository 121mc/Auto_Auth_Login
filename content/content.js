...
(async function () {
  ...
  async function dragSlider(elements, targetLeft, attempt) {
    ...
    // Preserve the configured trajectory timeline without flooding the task queue.
  ...
  async function submitAndSolveSlider(loginViewDiv) {
    ...
    throw new Error('Slider captcha did not appear after submission');
  }
  ...
  async function performAutoLogin(username, password) {
    ...
    // Step 4: Submit once, then complete the slider challenge.
    await submitAndSolveSlider(loginViewDiv);
